...
export class GwmWallImporter implements GwmItemImporter {
    ...
    public createItem(worldItem: GwmWorldItem, world: World): WorldItem {
        ...
        const dimensions = worldItem.dimensions.translate(new Point(translateX, translateY));
        // .negateY();

        const translate = new VectorModel(dimensions.getBoundingCenter().x, 2.5, -dimensions.getBoundingCenter().y);
    ...
    private makeWallNarrower(worldItem: GwmWorldItem) {
        ...
        if (rect.width > rect.height) {
            worldItem.dimensions = worldItem.dimensions.setBoundingHeight(1);
